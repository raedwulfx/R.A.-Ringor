...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      ...
      focus:outline-none focus:ring-2 focus:ring-[#e11d48] focus:ring-offset-2 
      focus:ring-offset-[#1c1e26] transition-all duration-300 peer
    `;

    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(248,113,113,0.4)] border-[#e11d48]"
      : "border-white/20 hover:border-[#e11d48]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#e11d48]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#e11d48] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#e11d48]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
